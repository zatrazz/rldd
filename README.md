...
The loader cache is read directly from /etc/ld.so.cache (both the old and the current format are supported) with the entries filtered by the object architecture and hwcap bits. Some ABI extends with withglibc-hwcaps extension, which is also supported (an entry in a glibc-hwcaps subdirectory is only used when the running CPU supports it, and the best-fit subdirectory wins, for instance x86-64-v2/-v3/-v4).  The default directories are the ones built into the loader of the object (its PT_INTERP, or for a shared library the loader of its architecture the cache lists), read from the loader file. For instance, /lib/x86_64-linux-gnu, /usr/lib/x86_64-linux-gnu, /lib and /usr/lib on a Debian x86_64 install, or /lib64 and /usr/lib64 on an upstream build.  Without a loader to read, the slibdir of the architecture and its /usr counterpart are assumed.
...
The Android backend is only compiled when cross building and needs a real system image to be exercised. The device test suite is under [tests/android](tests/android), which builds rldd for the ABI of each attached
...
The Mach-O backend takes the recorded load paths to the dyld shared cache, the filesystem or the OS cryptex of the running system, and it has a suite of its own under [tests/macos](tests/macos).  It runs rldd over the objects of the system directories and the dyld cache images and compares the dependency lists, and where each one resolved to, with ‘dyld_info -dependents’:
